...
interface Props {
  ...
  loadPaymentsByMonth:      (year: number, month: number) => Promise<PaymentRecord[]>;
...
function buildSlotUsers(sessions: SavedSession[], payments: PaymentRecord[], slotId: string, date: string): UserRow[] {
  const rows: UserRow[] = [];
  for (const session of sessions) {
    ...
    rows.push({ contact: session.contact, betTotal, amountPaid: pr?.amountPaid ?? null, commissionPct: pr?.commissionPct, paymentId: pid });
  }
  return rows.sort((a, b) => a.contact.localeCompare(b.contact));
}

interface DaySummary {
  date: string; totalBets: number; received: number; earned: number; pending: number;
}

function buildMonthData(
  sessions: SavedSession[], payments: PaymentRecord[],
  commissionPct: number, year: number, month: number,
) {
  ...
  for (const date of allDates) {
    ...
    const dayPayments = payments.filter(p => p.date === date && p.amountPaid !== null);
    const received = dayPayments.reduce((sum, p) => sum + (p.amountPaid ?? 0), 0);
    ...
    days.push({ date, totalBets, received, earned: Math.round(earned * 100) / 100, pending });
  }
  ...
  const totalEarned   = Math.round(days.reduce((s, d) => s + d.earned, 0) * 100) / 100;
...
export default function GamesView({
  ...
  const slotUsers    = (slotId: string) => buildSlotUsers(daySessions, dayPayments, slotId, selectedDate);
  const slotSummary  = (slotId: string) => {
    ...
    return { totalBets, received, myEarning: Math.round(myEarning * 100) / 100, count: users.length };
  };
  const daySummary = () => {
    ...
    return {
      ...
      myEarning: Math.round(all.reduce((s, x) => s + x.myEarning, 0) * 100) / 100,
    };
  };

  // ── Payment editing ────────────────────────────────────────────────────────

  const startEdit = (paymentId: string, current: number | null, currentPct?: number) =>
    setEditState({ id: paymentId, value: current !== null ? String(current) : "", pct: String(currentPct ?? settings.commissionPct), showPct: false });

  const saveEdit = async (paymentId: string, contact: string, slotId: string, slotName: string) => {
    ...
    const updatedPayments = upsertPayment(dayPayments, {
      id: paymentId, contact, slotId, slotName,
      date: selectedDate, amountPaid: amount, commissionPct,
    });
  ...
  const { days: monthDays, totalBets: monthBets, totalReceived: monthReceived, totalEarned: monthEarned, totalPending: monthPending } =
    buildMonthData(monthSessions, monthPayments, settings.commissionPct, monthYear.year, monthYear.month);
  ...
  return (
    <div className="w-full max-w-[640px] mx-auto">
      ...
      <div className="flex bg-white rounded-[16px] border-2 border-[#e4edf8] p-1 mb-5 shadow-sm">
        {(["daily", "monthly"] as ViewMode[]).map(mode => (
          <button
            ...
            className={`flex-1 py-3 text-[16px] font-bold rounded-[12px] transition-colors ${viewMode === mode ? "bg-[#1d6fb8] text-white shadow-sm" : "text-gray-400"}`}
          >
      ...
      {viewMode === "daily" && (
        <>
          ...
          <div className="flex items-center justify-between mb-4">
            <button onClick={() => setSelectedDate(d => shiftDate(d, -1))} className="w-14 h-14 flex items-center justify-center rounded-2xl bg-white border-2 border-[#dde8f8] text-[#1d6fb8] font-bold shadow-sm active:bg-[#e8f0fc] text-3xl">‹</button>
            ...
            <button onClick={() => { if (!isFuture) setSelectedDate(d => shiftDate(d, 1)); }} disabled={isFuture}
              className={`w-14 h-14 flex items-center justify-center rounded-2xl border-2 font-bold shadow-sm text-3xl ${!isFuture ? "bg-white border-[#dde8f8] text-[#1d6fb8] active:bg-[#e8f0fc]" : "bg-gray-50 border-gray-200 text-gray-300 cursor-not-allowed"}`}
            >›</button>
          </div>
            ...
            <>
              ...
              {(totalBets > 0 || dayReceived > 0) && (
                <div className="bg-[#1d6fb8] rounded-[20px] px-5 py-4 mb-4 shadow-[0_4px_20px_rgba(29,111,184,0.25)]">
                  <div className="text-[12px] font-bold text-white/60 uppercase tracking-widest mb-3 text-center">Day Summary</div>
                  <div className="grid grid-cols-2 gap-3">
                    {[{ label: "Total Game", value: totalBets }, { label: "I Received", value: dayReceived }].map(({ label, value }) => (
                ...
                {enabledSlots.map(slot => {
                  ...
                  return (
                    <div key={slot.id} className="bg-white rounded-[20px] shadow-sm border-2 border-[#e4edf8] overflow-hidden">
                      <button onClick={() => toggleSlot(slot.id)} className="w-full flex items-center gap-4 px-5 py-4 hover:bg-[#f5f9ff] active:bg-[#eef4ff] transition-colors text-left">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {summary.count > 0 ? (
                            <div className="text-[13px] text-gray-400 mt-1">
                              {summary.count} {summary.count === 1 ? "person" : "people"} · Received <span className="font-bold text-[#1d6fb8]">₹{summary.received}</span> of ₹{summary.totalBets}
                            </div>
                          ) : (
                        ...
                        <div className="flex flex-col items-end gap-2 shrink-0">
                          {isToday && <span className={`text-[11px] font-bold px-2.5 py-1 rounded-full ${badge}`}>{statusLabel}</span>}
                          {summary.myEarning > 0 && (
                            <span className="text-[12px] font-bold text-green-700 bg-green-50 border border-green-200 px-2.5 py-1 rounded-full">+₹{summary.myEarning}</span>
                          )}
                          ...
                          {users.length === 0 ? (
                            ...
                            <>
                              <div className="divide-y-2 divide-[#f0f4f8]">
                                {users.map(user => {
                                  const effectivePct = user.commissionPct ?? settings.commissionPct;
                                    ...
                                    <div key={user.contact} className="px-5 py-4">
                                      ...
                                      {isEditing ? (
                                        <div className="bg-[#f0f6ff] border-2 border-[#1d6fb8] rounded-[14px] p-4">
                                          ...
                                          {/* Commission % editor */}
                                          <button
                                            onClick={() => setEditState(s => s ? { ...s, showPct: !s.showPct } : s)}
                                            className="text-[12px] text-gray-400 underline mb-2 block"
                                          >
                                            {editState!.showPct ? "▲ Hide" : "▼ Change"} commission % (currently {editState!.pct}%)
                                          </button>
                                          {editState!.showPct && (
                                            <div className="flex items-center gap-2 mb-3">
                                              <input
                                                ...
                                                value={editState!.pct}
                                                onChange={e => setEditState(s => s ? { ...s, pct: e.target.value } : s)}
                                              ...
                                              <span className="text-[12px] text-gray-400">for this payment only</span>
                                            </div>
                                          )}

                                          <div className="flex gap-2">
                                            <button onClick={() => saveEdit(user.paymentId, user.contact, slot.id, slot.name)} className="flex-1 py-3 bg-green-600 text-white text-[16px] font-bold rounded-[12px] active:opacity-80">✓ Save</button>
                                            <button onClick={() => setEditState(null)} className="px-5 py-3 bg-gray-100 text-gray-500 text-[16px] font-semibold rounded-[12px] active:opacity-80">Cancel</button>
                                          </div>
                                        </div>
                                      ) : notRecorded ? (
                                        <button onClick={() => startEdit(user.paymentId, null, user.commissionPct)} className="w-full py-4 bg-[#1d6fb8] text-white text-[16px] font-bold rounded-[14px] flex items-center justify-center gap-2 active:opacity-80 shadow-sm">
                                        ...
                                        <div className="flex items-center justify-between bg-green-50 border-2 border-green-200 rounded-[14px] px-4 py-3">
                                          <div>
                                            ...
                                            <div className="text-[11px] text-gray-400 mt-0.5">Commission: {effectivePct}%</div>
                                          </div>
                                          <div className="flex flex-col items-end gap-1.5">
                                            <span className="text-[13px] font-bold text-green-700 bg-green-100 px-3 py-1 rounded-full">✅ Fully Paid</span>
                                            <button onClick={() => startEdit(user.paymentId, user.amountPaid, user.commissionPct)} className="text-[12px] text-gray-400 underline">Edit</button>
                                          </div>
                                        </div>
                                      ) : hasDebt ? (
                                        <div className="rounded-[14px] border-2 border-orange-200 overflow-hidden">
                                          <div className="flex items-center justify-between bg-orange-50 px-4 py-3">
                                            ...
                                            <button onClick={() => startEdit(user.paymentId, user.amountPaid, user.commissionPct)} className="text-[13px] text-gray-400 underline">Edit</button>
                                          </div>
      ...
      {viewMode === "monthly" && (
        <>
          <div className="flex items-center justify-between mb-4">
            <button onClick={() => shiftMonth(-1)} className="w-14 h-14 flex items-center justify-center rounded-2xl bg-white border-2 border-[#dde8f8] text-[#1d6fb8] font-bold shadow-sm active:bg-[#e8f0fc] text-3xl">‹</button>
            <div className="text-center">
              <div className="text-[22px] font-extrabold text-[#1a1a1a]">{MONTH_NAMES[monthYear.month - 1]}</div>
              <div className="text-[14px] text-gray-400">{monthYear.year}</div>
            </div>
            <button onClick={() => { if (canGoNextMonth) shiftMonth(1); }} disabled={!canGoNextMonth}
              className={`w-14 h-14 flex items-center justify-center rounded-2xl border-2 font-bold shadow-sm text-3xl ${canGoNextMonth ? "bg-white border-[#dde8f8] text-[#1d6fb8] active:bg-[#e8f0fc]" : "bg-gray-50 border-gray-200 text-gray-300 cursor-not-allowed"}`}
            >›</button>
          </div>
            ...
            <>
              ...
              {monthDays.length === 0 ? (
                <div className="bg-white rounded-[20px] border-2 border-[#e4edf8] px-5 py-12 text-center shadow-sm">
                  ...
                  <div className="text-[14px] text-gray-400 mt-1">for {MONTH_NAMES[monthYear.month - 1]} {monthYear.year}</div>
                </div>
              ) : (
                <div className="space-y-3">
                    ...
                    >
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        ...
                        <div className={`rounded-[12px] px-2 py-2.5 text-center ${day.pending > 0 ? "bg-orange-50" : "bg-gray-50"}`}>
                          ...
                          <div className={`text-[17px] font-extrabold leading-none ${day.pending > 0 ? "text-orange-600" : "text-gray-400"}`}>₹{day.pending}</div>
                        </div>
                      </div>
                    </button>
                  ))}
                  ...
                  <div className="bg-[#1a3a5c] rounded-[18px] px-4 py-4 shadow-[0_4px_16px_rgba(26,58,92,0.25)]">
                    <div className="text-[12px] font-bold text-white/60 uppercase tracking-widest mb-3">Month Total — {MONTH_NAMES[monthYear.month - 1]} {monthYear.year}</div>
                    <div className="grid grid-cols-3 gap-2">
                      {[{ label: "Received", value: monthReceived, color: "text-white" }, { label: "My Earning", value: monthEarned, color: "text-green-300" }, { label: "Pending", value: monthPending, color: "text-orange-300" }].map(({ label, value, color }) => (
